...
class ExamWizard(SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        ...
        answers = [ans for test, ans in formset_1.items()]
        context = self.get_context_data('exam')
        context.update({'time': self.request.POST.get('time')})


        self.request.session['correct_answers'] = {test: ans for test, ans in formset_1.items() if ans in test.get_answers()}
        correct_answers = {test: ans for test, ans in formset_1.items() if ans in test.get_answers()}

        # time = request.session.get('time')
        # max_result = len(formset_1) + (len(formset_2) * 2)
        result = len(correct_answers)
        ...
        if formset_2:
            dict_part2_points = {test: ans for test, ans in formset_2.items()}
            result += sum([int(point) for point in dict_part2_points.values() if point != None])
            geometry_result += sum([int(val) for key, val in dict_part2_points.items() if
                                    Test.objects.get(id=key.id).get_category_id() in [23, 24, 25] and val != None])
